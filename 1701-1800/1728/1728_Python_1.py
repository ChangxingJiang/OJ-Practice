...
class Solution:
    def canMouseWin(self, grid: List[str], catJump: int, mouseJump: int) -> bool:
        ...
        # ---------- 计算老鼠到达食物的唯一通道的拐弯的地方 ----------
        # 如果猫蹲在必经之路的拐弯的地方上不动，那么老鼠无论如何也不可能到达了，因为老鼠必须在拐点上停一步
        # 计算最短路径（必经之路一定在最短路径上）
        visited = {mouse}
        ...
        for _ in range(1000):
            ...
            # 猫移动：最优策略为走向食物点；如果步数相同，尽可能选择绝对距离更短的
            # 猫不选择前往必经之路拐点，是因为猫到任意必经之路拐点的距离已经超过了老鼠的距离
            cat_choice = (-1, -1)
...
if __name__ == "__main__":
    print(Solution().canMouseWin(grid=["####F", "#C...", "M...."], catJump=1, mouseJump=2))  # True
    print(Solution().canMouseWin(grid=["M.C...F"], catJump=1, mouseJump=4))  # True
    print(Solution().canMouseWin(grid=["M.C...F"], catJump=1, mouseJump=3))  # False
    print(Solution().canMouseWin(grid=["C...#", "...#F", "....#", "M...."], catJump=2, mouseJump=5))  # False
    print(Solution().canMouseWin(grid=[".M...",
                                       "..#..",
                                       "#..#.",
                                       "C#.#.",
                                       "...#F"], catJump=3, mouseJump=1))  # True
    print(Solution().canMouseWin(grid=["C...#", "...#F", "....#", "M...."], catJump=2, mouseJump=5))  # False

    # False
    print(Solution().canMouseWin(grid=["#..C...",
                                       "M....#.",
                                       "######.",
                                       "######.",
                                       "######.",
                                       "######F"],
                                 catJump=1, mouseJump=5))
    ...
    print(Solution().canMouseWin(grid=["...C.",
                                       ...
                                       "...#.",
                                       "...#.",
                                       "F..#."], catJump=1, mouseJump=5))

    # True (测试用例142)
    print(Solution().canMouseWin(grid=["#F..",
                                       "..#.",
                                       "..M.",
                                       "..C."], catJump=3, mouseJump=3))

    # False
    print(Solution().canMouseWin(grid=[".C.#",
                                       ".#..",
                                       "M..F"], catJump=4, mouseJump=1))
    ...
    # True (测试用例174)
    print(Solution().canMouseWin(grid=["........",
                                       "F...#C.M",
                                       "........"], catJump=1, mouseJump=2))

    # 自制用例
    # False
    print(Solution().canMouseWin(grid=["M....C......F"], catJump=9, mouseJump=3))

    # False
    print(Solution().canMouseWin(grid=["M....C......F"], catJump=3, mouseJump=8))
